...
class PaymentCaptureResponse(BaseModel):
    """Payment capture response schema."""
    payment_id: int
    status: str
...
async def create_payment(
    ...
    Creates a payment intent with the configured payment provider.
    Requires authentication and valid account ownership.
    """
...
async def capture_payment(
    ...
    Captures a previously authorized payment and updates account balance.
    """
    payment_service = PaymentService(session)
    try:
        from decimal import Decimal
...
async def get_account_payments(
    ...
    Returns list of payments associated with a specific account.
    """
    payment_service = PaymentService(session)
    try:
